...
LOCAL_SYN_DDOS_ACTIVE_THRESHOLD = 2200
   # Local threshold for deactivation of SYN Mitigation
LOCAL_SYN_DDOS_INACTIVE_THRESHOLD = 1700
   # Global threshold for activation of SYN Mitigation
GLOBAL_SYN_DDOS_ACTIVE_THRESHOLD = 3000
   # Global threshold for activation of SYN Mitigation
GLOBAL_SYN_DDOS_INACTIVE_THRESHOLD = 2400
   # Timeout (in seconds) to increase the time of mitigation
FIXED_TIMEOUT_TIME = 20 
...
class MergedDdosMitigation(app_manager.RyuApp):
    ...
    def _request_stats(self, datapath):
        ...
        match = ofparser.OFPMatch(eth_type = 0x0800,
                                  ip_proto = 6,
                                  state = FSM_T_COUNT_Normal.UNKNOWN_SYN,
                                  tcp_flags = F_SYN)

        req = ofparser.OFPFlowStatsRequest(datapath, 0, 7, ofproto.OFPP_ANY,
                                        ofproto.OFPG_ANY, cookie, cookie_mask,match)
        datapath.send_msg(req)
    ...
    def _syn_mitigation(self, datapaths):
        ...
        for dp in datapaths:
            ...
            ts_h=time.strftime("%d.%m. %H:%M:%S")
            LOG.info("%d %s Mitigation FSM has been loaded to Table 6 and 7 of datapath %d" %
                    (ts, ts_h, dp.id))
            # Don't forget to restart the number of old unknown syn in the controller
            self.old_unknown_syn[dp.id] = 0
        self.mitig_on = True
    ...
    def _syn_return_to_normal_mode(self, datapaths):
        ...
        for dp in datapaths:
            ...
            ts=time.mktime(time.localtime())
            ts_h=time.strftime("%d.%m. %H:%M:%S")
            LOG.info("%d %s Normal FSM has been loaded to Table 6 and 7 of datapath %d" %
                    (ts, ts_h, dp.id))
            # Don't forget to restart the number of old unknown syn in the controller
            self.old_unknown_syn[dp.id] = 0
        self.mitig_on = False
